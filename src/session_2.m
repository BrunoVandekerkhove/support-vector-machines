...
Xtest = X(2:2:end);
Ytest = Y(2:2:end);
%% 1.2a A simple example: the sinc function (regression)
% demofun
%
i = 1;
for gamma = [10 1E3 1E6]
    for sigma = [1E-2 1 1E2]
        parameters = {X, Y, 'function estimation', gamma, sigma, 'RBF_kernel'};
...
close all
%% 1.2b A simple example: the sinc function (regression)
parameters = {X, Y, 'f', [], [], 'RBF_kernel'};
N = 1; % Number of runs
gsimp = ones(1,N);
ssimp = ones(1,N);
csimp = ones(1,N);
...
%% 1.2c Application to the Bayesian framework
sigma = 0.4;
gamma = 10;
...
sigma = 1000; % Choose parameters wisely to prevent it from modelling noise
gamma = 0.1;
%[selected, ranking] = bay_lssvmARD({X, Y, 'f', gamma, sigma}) %#ok
for set = (dec2bin(1:2^3-1) - '0')'
    set %#ok
    parameters = {X(:, set > 0), Y, 'f', gamma, sigma, 'RBF_kernel'};
    cost = crossvalidate(parameters, 10,'mse') %#ok
end
figure
hold on
...
%% 1.4a Robust regression (non-robust)
