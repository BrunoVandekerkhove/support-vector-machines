...
\par It was stated previously that one way to tune kPCA is to measure reconstruction errors for the validation set to gauge the effectiveness of certain parameter combinations. A plot of the reconstruction errors measured for the provided training and validation sets is provided in figure \ref{uspserror}. The errors tell a meaningful story which also corresponds to the visualisation of the performance of various parameter combinations in figure \ref{uspsband}.
...
\begin{figure}[h]
\centering
\subfloat[Errors (root mean squared errors) for the training set.]{\includegraphics[width=0.32\textwidth]{../src/figures/usps/contour_xtest1}}\hfil
...
\caption{Reconstruction error for the training and validation sets in function of the bandwidth and the number of components. This is a crude grid search. More appropriate tuning methods include kernel parallel analysis or Model selection based on Distance Distributions (MDD).}
\label{uspserror}
\end{figure}
...
\begin{minipage}{\linewidth}
    ...
    \includegraphics[width=\linewidth]{../src/figures/fixedsize/fixedsize_1}
\end{minipage}
\caption{Denoising of the USPS dataset using tuned parameters.}
\label{fixedsample}
\end{wrapfigure}
