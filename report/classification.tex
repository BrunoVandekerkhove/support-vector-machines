...
With 30 features for each of the observations in the dataset the Wisconsin breast cancer isn't trivial to visualise. \texttt{MATLAB} has an implementation of t-SNE or two principal components determined through PCA could be plotted. In contrast with the previous dataset this one's a bit unbalanced ($62.5\%$ belongs to the negative class) which can be accounted for through bias correction or with weights in the SVM formulation. 
