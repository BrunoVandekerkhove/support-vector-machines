...
The results with a similar tuning method (this time by using cross-validation rather than making use of the Bayesian network) are shown in figure \ref{santafeord}.

\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{../src/figures/santafe/mseval}
%\includegraphics[width=\textwidth]{../src/figures/santafe/msestest}
\caption{Mean squared errors for various models in function of the order. Parameters were tuned automatically using a grid search. Outliers (large errors) are removed to improve the visualisation.}
\label{santafeord}
\end{figure}
