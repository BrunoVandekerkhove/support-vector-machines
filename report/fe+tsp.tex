...
\par Up til now only LS-SVMs were toyed with. It's also possible to work with classical SVMs which - due to the choice of loss function - provide sparsity without necessitating a pruning post-processing step or some other approach to enforce it. The provided \texttt{MATLAB} library has one interesting kernel that is said to perform well for regression, i.e. the ANOVA kernel.
