...
The provided model for predicting measurements for the logmap dataset performs quite badly as can be seen below. The results are poor as none of the 3 parameters are tuned.
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{../src/figures/logmap/mse}
\caption{\texttt{mse} of just one tuned model per possible order. A minimum of 0.0408 was obtained for \texttt{order} = 23. The experiment was repeated thrice. When doing the same experiment where the \texttt{mse} was measured for the test set the results weren't nearly the same. Minimum errors were found for orders $\in[2,8]$.}
\label{logmapord}
\end{figure}

\par Figure \ref{logmapord} shows the mean squared errors in function of the order for LS-SVMs using RBF kernels with tuned parameters. This was measured for the test set. The validation set gave a different story where the error remained stable up until an order of about 8.
